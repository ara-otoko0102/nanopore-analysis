...
# ═══════════════════════════════════════════════════════════════
# MIL Model  (平均プーリングで集約)
#   h_i → 1D-CNN → H [N, 32]
#   M = mean(H, dim=0)  [1, 32]
#   output = Sigmoid(Linear(M))
# ═══════════════════════════════════════════════════════════════
class MILModel(nn.Module):
    ...
    def forward(self, bag_of_waveforms):
        ...
        M      = H.mean(dim=0, keepdim=True)   # (1, 32) 平均プーリング
...
XA, YA = shuffled(X1 + X4, y1 + y4)
XB, YB = shuffled(X2 + X5, y2 + y5)
XC, YC = shuffled(X3 + X6, y3 + y6)

print("C")
split_idx        = int(len(XC) * 0.8)
X_train, y_train = XC[:split_idx], YC[:split_idx]
X_test,  y_test  = XC[split_idx:], YC[split_idx:]
...
# ═══════════════════════════════════════════════════════════════
# Test & Cross-condition Evaluation
# ═══════════════════════════════════════════════════════════════
